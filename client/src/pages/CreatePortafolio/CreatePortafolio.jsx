...
export default function CreatePortafolio() {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (response.data) {
                navigate('/portafolio/' + response.data.id);
            }
        } catch (error) {
                        ...
                        {collectionsFields.map((field, index) => (
                            <div className="flex flex-col gap-5" key={field.id}>
                                ...
                                {photoFields.map((field, index) => (
                                    <div key={field.id} className="grid grid-col-4 gap-2">
                                        <div className="flex flex-col">
                                            ...
                                            <input 
                                                ...
                                                name="price"
                                                placeholder="Precio"
                                                {...register(`photos.${index}.price`, { required: true })}
                                            />
                                            <label>
                                            <input 
                                                type="checkbox" 
                                    ...
                                    <button type="button" onClick={() => appendPhoto({ image: '',title: '' ,description: '', is_public: '', is_free: '', price:'' })}>
                                        Añadir nueva foto
                                    </button>
